...
def intro_ask_what_to_do():
    global two_btns_label, create_room_btn, join_room_btn, btn_titulo

    #Titulo monopoly
    btn_titulo = tk.Label(start_frame, text="Monopoly", font = ("Constantia", 50), fg="red")
    btn_titulo.grid(row=0, column=2, columnspan=3)

    two_btns_label = tk.Label(start_frame, text="Que desea hacer?", font=font)
    two_btns_label.grid(row=4, column=2, columnspan=3)
    # give our client a choice of creating or joining a room
    create_room_btn = tk.Button(start_frame, text="Crear sala.", command=lambda: create_room(), width=12, height=3, font = ("Constantia", 13))
    create_room_btn.grid(row=8, column=3)
    join_room_btn = tk.Button(start_frame, text="Unirse a sala.", command=lambda: join_room(), width=12, height=3, font = ("Constantia", 13))
    join_room_btn.grid(row=10, column=3)
...
def create_room():
    global player_desig, btn_volver
    #Boton para volver atras.
    btn_volver = tk.Button(start_frame, text="Volver.", command=lambda: clear_and_home(), width=5, height=1, font = ("Constantia", 10))
    btn_volver.grid(row=8, column=3)
    player_desig = "host"
...
def ask_username():
    ...
    username_label = tk.Label(start_frame, text="Ingrese su nombre de usuario.", font=font)
    username_label.grid(row=2, column=3)
    username_entry = tk.Entry(start_frame)
    ok_but_for_username = tk.Button(start_frame, text="Listo", font=font, command=lambda: ok_but_for_username_clicked())
